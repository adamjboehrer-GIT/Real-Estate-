...
> **Revision 2026-05-08 (final pass)** — Reworked the entire postcard to match the visual language of Adam's existing `StateOfNeighborhood_Mailer_9x6_0413_Print.pdf`. Specific changes: (a) display serif swapped to **Fraunces** (high-contrast fashion serif, much closer to the State mailer's display face); (b) card background changed to warm cream `#EDE7DC` matching the State mailer; (c) body copy switched from italic serif to upright Source Sans 3 — italic-serif body felt overly affected for this volume of copy; (d) eyebrow color switched to warm tan `#A89878` matching the State mailer's tone-on-tone aesthetic; (e) horizontal rules switched from gold to warm grey `#C8C0B2` (the gold rules read too prominent on cream); (f) the hand-drawn marker annotation, curved gold arrow, and Permanent Marker handwriting next to the QR were all removed — they conflict with the clean editorial style of the State mailer. The QR is now a clean panel with a tan-eyebrow caption "HOME VALUE REPORT" above and a brief sans pitch line below.
>
> **Revision 2026-05-08 (afternoon)** — Display serif swapped to Newsreader (replaced later in the day by Fraunces — see final pass above).
>
> **Revision 2026-05-08** — Three changes: (1) body copy rewritten — the prior body ("Most coastal sales don't start on Zillow. A buyer comes to me with an idea of what they want…") was retired because it implied a buyer would arrive off-market, which works against the seller-listing goal of the mailer; the new body keeps the "conversation before the listing" headline but routes the value to better positioning at list time, not buyer delivery. (2) Card background changed to soft light grey `#E8E8E8` — the prior mockup used a warm cream (`#f4f1ec`) which is off-brand; PSIR Design Guidelines permit White, SIR Blue, or Grey as background colors, and the soft grey gives the card a luxury feel while keeping SIR Blue text crisp. (3) The Pacific Sotheby's International Realty lockup is now placed as the actual brand logo (`assets/psir-logo.png`), replacing the previous CSS-typeset approximation.
>
...
5. **Body copy length (revised 2026-05-08)** — the italic-serif body is now four sentences (roughly the same character count as the 2026-05-04 version). At 17.5 pt it should fit cleanly above the QR; if the layout feels crowded after typesetting, please propose either a slight pt reduction (down to 16 pt) or a small re-line to keep the SIR Blue headline as the dominant element.
